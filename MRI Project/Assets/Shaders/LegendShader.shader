...
Shader "Custom/LegendFilter" {
	...
	Properties{
		...
		_UseAlpha("Transfer Function", Vector) = (0,0,0,0)
...
#include "UnityCG.cginc"
	...
	//STEP_CNT has large effect on frame rate. My computer can barely handle 200, but others can easily do 400
	#define STEP_CNT 200 // should ideally be at least as large as data resolution, but strongly affects frame rate
		...
		float4 frag(frag_input i) : COLOR
		{
			...
			[unroll(STEP_CNT)] for (int k = 0; k < STEP_CNT; k++) {
				...
				// apply transfer function passed in thru _UseAlpha
				voxel_col.a = max(min(_UseAlpha.x*voxel_col.x + _UseAlpha.y, 1), 0);
				voxel_col.rgb = max(min(_UseAlpha.z*voxel_col.x + _UseAlpha.w, 1), 0);

				// multiplier is 1 if distance is between startDepth and stopDepth, otherwise 0.
				float multiplier = step(startDepth, distance) * step(distance, stopDepth);

				// update current ray color using A and 1-A blending
				ray_col.rgb = ray_col.rgb + (1 - ray_col.a) * voxel_col.a * voxel_col.rgb * multiplier;
				ray_col.a = ray_col.a + (1 - ray_col.a) * voxel_col.a * multiplier;
